...
class ErrorFactory {
    ...
    create(e, ...t) {
        ...
        }(o, n) : "Error", s = `${this.serviceName}: ${i} (${a}).`;
...
function openDB(e, t, {blocked: n, upgrade: r, blocking: a, terminated: o} = {}) {
...
const l = ["get", "getKey", "getAll", "getAllKeys", "count"], g = ["put", "add", "delete", "clear"], f = new Map;

function getMethod(e, t) {
    if (!(e instanceof IDBDatabase) || t in e || "string" != typeof t) return;
    if (f.get(t)) return f.get(t);
    const n = t.replace(/FromIndex$/, ""), r = t !== n, a = g.includes(n);
...
const h = "@firebase/installations", w = new ErrorFactory("installations", "Installations", {
...
async function set(e, t) {
    const n = getKey$1(e), r = (await getDbPromise$1()).transaction(v, "readwrite"), a = r.objectStore(v),
        o = await a.get(n);
    return await a.put(t, n), await r.done, o && o.fid === t.fid || fidChanged(e, t.fid), t
}

async function remove(e) {
    const t = getKey$1(e), n = (await getDbPromise$1()).transaction(v, "readwrite");
    await n.objectStore(v).delete(t), await n.done
}

async function update(e, t) {
    const n = getKey$1(e), r = (await getDbPromise$1()).transaction(v, "readwrite"), a = r.objectStore(v),
        o = await a.get(n), i = t(o);
...
async function getInstallationEntry(e) {
    ...
    const n = await update(e.appConfig, (n => {
        const r = function updateOrCreateInstallationEntry(e) {
            ...
            if (0 === t.registrationStatus) {
                if (!navigator.onLine) {
                    return {installationEntry: t, registrationPromise: Promise.reject(w.create("app-offline"))}
                }
                    ...
                    r = async function registerInstallation(e, t) {
                        try {
                            const n = await async function createInstallationRequest({
                                ...
                                const r = getInstallationsEndpoint(e), a = getHeaders$1(e),
                                    o = t.getImmediate({optional: !0});
                                if (o) {
                                    const e = await o.getHeartbeatsHeader();
                                    e && a.append("x-firebase-client", e)
                                }
                                const i = {fid: n, authVersion: "FIS_v2", appId: e.appId, sdkVersion: "w:0.6.4"},
    ...
    return "" === n.fid ? {installationEntry: await t} : {installationEntry: n, registrationPromise: t}
}
...
async function waitUntilFidRegistration(e) {
    ...
    if (0 === t.registrationStatus) {
        const {installationEntry: t, registrationPromise: n} = await getInstallationEntry(e);
        return n || t
    }
    return t
}
...
function clearTimedOutRequest(e) {
    ...
    }(e) ? {fid: e.fid, registrationStatus: 0} : e
}

async function generateAuthTokenRequest({appConfig: e, heartbeatServiceProvider: t}, n) {
    ...
    const i = {installation: {sdkVersion: "w:0.6.4", appId: e.appId}},
        s = {method: "POST", headers: a, body: JSON.stringify(i)}, c = await retryIfServerError((() => fetch(r, s)));
    if (c.ok) {
...
const publicFactory = e => {
    const t = e.getProvider("app").getImmediate(), n = function extractAppConfig$1(e) {
        if (!e || !e.options) throw getMissingValueError$1("App Configuration");
        if (!e.name) throw getMissingValueError$1("App Name");
        const t = ["projectId", "apiKey", "appId"];
        for (const n of t) if (!e.options[n]) throw getMissingValueError$1(n);
    ...
    const t = e.getProvider("app").getImmediate(), n = _getProvider(t, "installations").getImmediate();
    return {
        getId: () => async function getId(e) {
            const t = e, {installationEntry: n, registrationPromise: r} = await getInstallationEntry(t);
            return r ? r.catch(console.error) : refreshAuthToken(t).catch(console.error), n.fid
        }(n), getToken: e => getToken$2(n, e)
    }
};
...
function base64ToArray(e) {
    const t = (e + "=".repeat((4 - e.length % 4) % 4)).replace(/\-/g, "+").replace(/_/g, "/"), n = atob(t),
        ...
        const t = await async function migrateOldDatabase(e) {
            ...
            return (await openDB("fcm_token_details_db", 5, {
                upgrade: async (n, r, a, o) => {
                    ...
                    const s = o.objectStore("fcm_token_object_Store"), c = await s.index("fcmSenderId").get(e);
                    if (await s.clear(), c) if (2 === r) {
                        const e = c;
            ...
            })).close(), await deleteDB("fcm_token_details_db"), await deleteDB("fcm_vapid_details_db"), await deleteDB("undefined"), function checkTokenDetails(e) {
...
async function dbSet(e, t) {
    const n = getKey(e), r = (await getDbPromise()).transaction(E, "readwrite");
...
async function requestDeleteToken(e, t) {
    ...
    try {
        ...
        throw C.create("token-unsubscribe-failed", {errorInfo: null == e ? void 0 : e.toString()})
    }
}
...
async function getTokenInternal(e) {
    const t = await async function getPushSubscription(e, t) {
        ...
        if (n) return n;
        return e.pushManager.subscribe({userVisibleOnly: !0, applicationServerKey: base64ToArray(t)})
    ...
    if (r) {
        if (function isTokenValid(e, t) {
            const n = t.vapidKey === e.vapidKey, r = t.endpoint === e.endpoint, a = t.auth === e.auth,
                o = t.p256dh === e.p256dh;
            return n && r && a && o
            ...
            try {
                const n = await async function requestUpdateToken(e, t) {
                        const n = await getHeaders(e), r = getBody(t.subscriptionOptions),
                        ...
                        try {
                            ...
                            throw C.create("token-update-failed", {errorInfo: null == e ? void 0 : e.toString()})
                        }
                        if (o.error) {
                            const e = o.error.message;
                            throw C.create("token-update-failed", {errorInfo: e})
                        }
        ...
        }(e, {token: r.token, createTime: Date.now(), subscriptionOptions: n}) : r.token;
        try {
...
async function deleteTokenInternal(e) {
    ...
    t && (await requestDeleteToken(e.firebaseDependencies, t.token), await async function dbRemove(e) {
        const t = getKey(e), n = (await getDbPromise()).transaction(E, "readwrite");
...
async function getNewToken(e, t) {
    const n = await async function requestGetToken(e, t) {
        const n = await getHeaders(e), r = getBody(t), a = {method: "POST", headers: n, body: JSON.stringify(r)};
        let o;
        try {
            const t = await fetch(getEndpoint(e.appConfig), a);
            o = await t.json()
        } catch (e) {
            throw C.create("token-subscribe-failed", {errorInfo: null == e ? void 0 : e.toString()})
        }
        if (o.error) {
            const e = o.error.message;
        ...
        return o.token
    }(e, t), r = {token: n, createTime: Date.now(), subscriptionOptions: t};
...
async function registerDefaultSw(e) {
    try {
        ...
        throw C.create("failed-service-worker-registration", {browserErrorMessage: null == e ? void 0 : e.message})
    }
}
...
const M = "@firebase/messaging", WindowMessagingFactory = e => {
